...
import PRNTab from './tabs/PRNTab'

// ── Tab bar ───────────────────────────────────────────────────────
const TABS = [
  { key: 'inbound', label: 'Inbound'                },
  { key: 'pr',      label: 'Purchase Requisition'   },
  { key: 'grn',     label: 'GRN'                    },
  { key: 'prn',     label: 'PRN'                    },
]
      ...
      {TABS.map(t => (
        <button
          ...
          style={{
            ...
            color: active === t.key ? '#37352F' : 'rgba(55,53,47,0.55)',
            background: 'none',
            border: 'none',
            borderBottom: active === t.key ? '2px solid #37352F' : '2px solid transparent',
            cursor: 'pointer',
            marginBottom: -1,
...
function InboundEntryTab() {
  ...
  const { data: entries, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await q.limit(500); return (data || []) as any[]
    },
  })
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success(editing ? 'Updated' : 'Saved'); qc.invalidateQueries({ queryKey: ['inbound'] }); qc.invalidateQueries({ queryKey: ['dashboard-stats'] }); setOpen(false) },
        ...
        {isLoading ? <LoadingSpinner /> : !entries?.length ? (
          ...
          <div style={{ overflowX:'auto' }}>
            <table className="notion-table">
              <thead><tr>
                <th style={{width:36}}>#</th><th>Date</th><th>SAP Invoice</th><th>Party</th>
                <th>Route</th><th style={{textAlign:'right'}}>Fridge</th><th style={{textAlign:'right'}}>WM</th>
                <th style={{textAlign:'right'}}>MWO</th><th style={{textAlign:'right'}}>AC</th>
                <th style={{textAlign:'right'}}>Total</th><th>Vehicle</th><th style={{textAlign:'right'}}>Cost</th>
                <th>Month</th><th style={{width:60}}></th>
              </tr></thead>
              <tbody>
                {entries?.map((row, i) => (
                  <tr key={row.id} className="group">
                    <td style={{color:'rgba(55,53,47,0.50)',fontSize:11,fontFamily:'monospace'}}>{i+1}</td>
                    ...
                    <td><div style={{fontWeight:500,fontSize:13,maxWidth:140}} className="truncate">{row.party_name}</div>{row.party_code&&<div style={{fontSize:11,color:'rgba(55,53,47,0.50)'}}>{row.party_code}</div>}</td>
                    <td style={{fontSize:12,color:'rgba(55,53,47,0.65)',whiteSpace:'nowrap'}}>{row.load_from} → {row.offload_to}</td>
                    <td style={{textAlign:'right',fontWeight:500,color:'#1d6fa8'}}>{row.refrigerator_qty||0}</td>
                    <td style={{textAlign:'right',fontWeight:500,color:'#5b3692'}}>{row.washing_machine_qty||0}</td>
                    <td style={{textAlign:'right',fontWeight:500,color:'#a07800'}}>{row.microwave_oven_qty||0}</td>
                    <td style={{textAlign:'right',fontWeight:500,color:'#0a6b5c'}}>{row.air_conditioner_qty||0}</td>
                    <td style={{textAlign:'right',fontWeight:700}}>{total(row)}</td>
                    <td style={{fontSize:12,color:'rgba(55,53,47,0.65)'}}>{row.vehicle_no||'—'}</td>
                    ...
                    <td><RowActions>
                      ...
                      <button onClick={()=>{if(confirm('Delete?'))del.mutate(row.id)}} className="btn-ghost" style={{color:'#c0392b'}}><Trash2 size={12}/></button>
                    </RowActions></td>
                  </tr>
                ))}
              </tbody>
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title={editing?'Edit Inbound Entry':'New Inbound Entry'} size="xl">
        <form onSubmit={e=>{e.preventDefault();if(validate())save.mutate()}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <FormField label="Receiving Date" required error={errors.receiving_date}><input type="date" value={form.receiving_date} onChange={set('receiving_date')}/></FormField>
            ...
            <FormField label="Load From" required error={errors.load_from}><input value={form.load_from} onChange={set('load_from')} list="wh-from-in"/><datalist id="wh-from-in">{WAREHOUSES.map(w=><option key={w} value={w}/>)}</datalist></FormField>
            <FormField label="Offload To" required error={errors.offload_to}><input value={form.offload_to} onChange={set('offload_to')} list="wh-to-in"/><datalist id="wh-to-in">{WAREHOUSES.map(w=><option key={w} value={w}/>)}</datalist></FormField>
            ...
            <FormField label="Vehicle No."><input value={form.vehicle_no} onChange={set('vehicle_no')} placeholder="e.g. DM TA-11-5329"/></FormField>
          ...
          <div style={{marginTop:16,paddingTop:16,borderTop:'1px solid rgba(55,53,47,0.09)'}}>
            <div style={{fontSize:11,fontWeight:600,color:'rgba(55,53,47,0.55)',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:10}}>Quantities Received</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
              ...
              <FormField label="Air Conditioner"><input type="number" min={0} value={form.air_conditioner_qty} onChange={set('air_conditioner_qty')}/></FormField>
            </div>
          </div>
  ...
  return (
    <div>
      <PageHeader
        icon="call_received"
        title="Inbound"
        subtitle="Receiving, Purchase Requisition, GRN & Returns"
      />
      <TabBar active={tab} onChange={setTab} />
      {tab === 'inbound' && <InboundEntryTab />}
      {tab === 'pr'      && <PRTab />}
      {tab === 'grn'     && <GRNTab />}
      {tab === 'prn'     && <PRNTab />}
    </div>
