...
import {
  ...
  Database, Users, BarChart2, Boxes, Menu, X, Truck,
  ChevronRight, ScanLine
} from 'lucide-react'

const NAV_OPERATIONS = [
  { to: '/', label: 'Dashboard', icon: LayoutDashboard },
  { to: '/serial', label: 'Serial Search', icon: ScanLine },
...
const ALL_NAV = [...NAV_OPERATIONS, ...NAV_MASTER]

export default function Layout({ children }: { children: React.ReactNode }) {
  const [open, setOpen] = useState(false)
  const location = useLocation()
  const pageName = ALL_NAV.find(n => n.to === location.pathname)?.label ?? 'WBL Operations'
    ...
    <div className="min-h-screen flex bg-slate-50">
      ...
      `}>
        ...
        <nav className="flex-1 py-4 overflow-y-auto">
          ...
          {NAV_OPERATIONS.map(({ to, label, icon }) => (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
          <button onClick={() => setOpen(!open)} className="lg:hidden p-2 rounded-lg text-slate-500 hover:bg-slate-100">
